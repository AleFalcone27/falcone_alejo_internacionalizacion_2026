...
export class PedidosCocineroPage implements OnInit {
  ...
  async confirmarTiempo() {
    if (!this.tiempoIngresado || this.tiempoIngresado <= 0) {
      AppComponent.instance.toast.show(this.translate.instant('PEDIDOS_COCINERO.INVALID_TIME'), 2000);
      return;
    }

    await this.authService.marcarProductoEnPreparacion(this.productoSeleccionado.id, this.tiempoIngresado);
    AppComponent.instance.toast.show(this.translate.instant('PEDIDOS_COCINERO.PRODUCT_TAKEN', { value: this.tiempoIngresado }), 2000);
  ...
  async marcarComoListo(item: any) {
    ...
    AppComponent.instance.toast.show(this.translate.instant('PEDIDOS_COCINERO.PRODUCT_READY'), 2000);
    this.cargarProductosPendientes();
  }
